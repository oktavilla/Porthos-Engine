      ...
      module Plugin
        ...
        module InstanceMethods
          ...
          def tag_names(namespace = nil)
            tags(namespace).collect { |tag| tag.name.include?(Porthos::MongoMapper::Taggable.delimiter) ? "\"#{tag.name}\"" : tag.name }.join(Porthos::MongoMapper::Taggable.delimiter)
          end
          ...
          def tag_names=(in_tags, namespace = nil)
            ...
            self._tags += tags_array.collect { |tag_name| Porthos::MongoMapper::Tag.new(:name => tag_name, :namespace => namespace)}
            @tags_changed = true
          end
        ...
        protected
          def split_tags(value)
            tag_list = []
            [/\s*#{Porthos::MongoMapper::Taggable.delimiter}\s*(['"])(.*?)\1\s*/, /^\s*(['"])(.*?)\1\s*#{Porthos::MongoMapper::Taggable.delimiter}?/].each do |exp|
              value.gsub!(exp) { tag_list << $2; "" }
            end
            tag_list + value.strip.split(Porthos::MongoMapper::Taggable.delimiter).collect(&:strip).uniq.compact
          end
...
MongoMapper::Document.plugin(Porthos::MongoMapper::Taggable::Plugin)