...
class DatumTest < ActiveSupport::TestCase
  ...
  test 'uniqueness of handle within collection' do
    parent = FactoryGirl.build(:page, :data => [])
    FactoryGirl.build(:string_field, :handle => 'le_handle').tap do |datum|
      parent.data << datum
  ...
  test 'building from a datum template' do
    template = FactoryGirl.build(:rich_text_field_template)
  ...
  test "knowing it's parent datum that is a direct child to page" do
    page = FactoryGirl.build(:page, :data => [FactoryGirl.build(:datum_collection, :handle => 'article')])
    decendant = FactoryGirl.build(:string_field)
  ...
  test "knowing if it's a direct child to page" do
    page = FactoryGirl.build(:page, :data => [FactoryGirl.build(:datum_collection, :handle => 'article')])
    child = FactoryGirl.build(:string_field)
    decendant = FactoryGirl.build(:string_field)
  ...
  test "delegates updated_at to its root document" do
    page = FactoryGirl.create(:page, :data => [])
    child = FactoryGirl.build(:string_field)
    page.data << child
